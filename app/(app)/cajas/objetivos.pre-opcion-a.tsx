/**
 * Copia intacta de la pantalla Objetivos antes del rediseño Opción A.
 * No editar: sirve como respaldo para revertir desde objetivos.tsx
 */
import {
  View,
  Text,
...
export default function ObjetivosScreen() {
  ...
  // Ventas por horas (pantalla flotante)
        ...
        <View style={styles.formRow}>
          <View style={[styles.formGroup, isNarrow && styles.formGroupNarrow]}>
            <SelectorDesplegable
              ...
              valorId={localSeleccionado ? String(localSeleccionado.id_Locales ?? (localSeleccionado.agoraCode ?? localSeleccionado.AgoraCode ?? '')) : null}
              ...
              onSeleccionar={(id) => {
                ...
                if (loc) setLocalSeleccionado(loc);
              }}
            />
          </View>
          <TouchableOpacity
      ...
      {registros.length > 0 ? (
        <View style={[styles.widget, styles.mediasPorDiaWidget, isNarrow && styles.widgetNarrow]}>
          ...
          <View style={styles.widgetLocalesHeader}>
            ...
            <View style={styles.widgetLocalesActions} {...{ dataSet: { captureHide: 'true' } }}>
            <TouchableOpacity
              style={styles.agrupBtn}
              onPress={() => setAgrupModalOpen(true)}
              accessibilityLabel="Gestionar agrupaciones"
            >
              <MaterialIcons name="workspaces" size={14} color="#7c3aed" />
            </TouchableOpacity>
                ...
                {localSeleccionado && (
                  <View style={styles.progressLocalRow}>
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() => registros.length > 0 && abrirHorasModal()}
                      disabled={registros.length === 0}
                      accessibilityLabel="Ventas por horas"
                    >
                      <MaterialIcons name="schedule" size={16} color={registros.length === 0 ? '#cbd5e1' : '#0369a1'} />
