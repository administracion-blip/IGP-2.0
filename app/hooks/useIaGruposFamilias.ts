/**
 * Agrupaciones de familias Ágora para Informes IA (persistidas en servidor).
 * Espejo de useMiaGruposFamilias con endpoints /api/ia/*.
 */
import { useCallback, useState } from 'react';
import { apiFetch, errorMessage } from '../utils/api';

export type IaFamilia = {
  id: string;
  nombre: string;
};

export type IaGrupoFamilias = {
  id: string;
  nombre: string;
  familiaIds: string[];
  orden: number;
...
function mapGrupo(raw: Record<string, unknown>): IaGrupoFamilias | null {
...
export function useIaGruposFamilias() {
  const [grupos, setGrupos] = useState<IaGrupoFamilias[]>([]);
  const [familias, setFamilias] = useState<IaFamilia[]>([]);
  ...
  const cargarGrupos = useCallback(async (opts?: { todos?: boolean }) => {
    ...
    try {
      ...
      const r = await apiFetch(`/api/ia/grupos-familias${qs}`);
      ...
      const list = (Array.isArray(data.grupos) ? data.grupos : [])
        ...
        .filter(Boolean) as IaGrupoFamilias[];
      list.sort(
        (a, b) =>
  ...
  const cargarFamilias = useCallback(async () => {
    ...
    try {
      const r = await apiFetch('/api/ia/familias');
      const data = await r.json();
      if (!r.ok || data.error) {
      ...
      const list: IaFamilia[] = (Array.isArray(data.familias) ? data.familias : [])
        .map((f: { id?: string | number; nombre?: string }) => ({
          id: String(f.id ?? '').trim(),
          nombre: String(f.nombre || f.id || '').trim(),
        }))
        .filter((f: IaFamilia) => f.id);
  ...
  const guardarGrupo = useCallback(
    async (input: {
      ...
      orden?: number;
      activo?: boolean;
    }): Promise<IaGrupoFamilias> => {
      const r = await apiFetch('/api/ia/grupos-familias', {
        method: 'PUT',
  ...
  const borrarGrupo = useCallback(async (id: string): Promise<void> => {
    ...
    const r = await apiFetch(`/api/ia/grupos-familias/${encodeURIComponent(sid)}`, {
      method: 'DELETE',
    });
