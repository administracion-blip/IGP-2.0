/**
 * Modal simple de agrupaciones de familias para Informes IA · ventas por artículo.
 */
...
import type { IaFamilia, IaGrupoFamilias } from '../../hooks/useIaGruposFamilias';

type Props = {
  visible: boolean;
  onClose: () => void;
  grupos: IaGrupoFamilias[];
  familias: IaFamilia[];
  /** Familias marcadas en el filtro (para precargar al crear). */
  familiaIdsIniciales?: string[];
  loading?: boolean;
  onGuardar: (input: {
    id?: string;
    nombre: string;
...
export function IaGruposFamiliasModal({
  visible,
  onClose,
  grupos,
  familias,
  ...
  const abrirNueva = () => {
    ...
    setEditId(undefined);
    setNombre('');
    setSeleccionados(new Set(familiaIdsIniciales));
    setFiltroFam('');
    setError(null);
  };

  const abrirEdicion = (g: IaGrupoFamilias) => {
    setEditando(true);
    setEditId(g.id);
    setNombre(g.nombre);
  ...
  const borrar = async (g: IaGrupoFamilias) => {
    ...
    try {
      await onBorrar(g.id);
      if (editId === g.id) cerrarFormulario();
    } catch (e) {
          ...
          {loading && !editando ? (
            ...
            <>
              <ScrollView style={styles.lista} nestedScrollEnabled>
                {grupos.length === 0 ? (
                  <Text style={styles.vacio}>
                    Aún no hay agrupaciones. Crea una con las familias marcadas en el filtro, o
                    elige familias aquí.
                  </Text>
                ) : (
                  grupos.map((g) => (
              ...
              >
                ...
                <Text style={styles.nuevaBtnText}>
                  {familiaIdsIniciales.length > 0
                    ? `Nueva (con ${familiaIdsIniciales.length} marcada${familiaIdsIniciales.length === 1 ? '' : 's'})`
                    : 'Nueva agrupación'}
                </Text>
              </TouchableOpacity>
            </>
...
const styles = StyleSheet.create({
  ...
  checkText: { fontSize: 13, color: '#475569', flex: 1 },
