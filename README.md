...
2. **URL del API en la app**  
   En la raíz del proyecto crea `.env` (opcional, por defecto usa `http://localhost:3002`, igual que `PORT` en `api/server.js`):

   - `EXPO_PUBLIC_API_URL=http://localhost:3002` (o la URL de tu API en producción)
...
Arranca el API (puerto 3002 por defecto) y la app web a la vez. Evita el error `ERR_CONNECTION_REFUSED` que aparece cuando el API no está en marcha.
...
1. Asegúrate de que el API esté corriendo (puerto 3002 por defecto; debe coincidir con `EXPO_PUBLIC_API_URL`).
